...
import (
	...
	"github.com/yguilai/go-cloud-storage/service/dbproxy/orm"
...
func DoUploadHandler(c *gin.Context) {
	...
	newFile.Seek(0, 0)
	fileMeta.FileSha1, err = kodo.FileHash(newFile)
	if err != nil {
		log.Printf("Failed to calc file hash, err:%s\n", err.Error())
		errCode = -4
		return
	}
	...
	} else if cfg.CurrentStoreType == cmn.StoreKODO {
		// 文件写入KODO存储
		key := cfg.KODORootDir + fileMeta.FileSha1
		// 判断写入KODO为同步还是异步
		if !cfg.AsyncTransferEnable {
			ret, err := kodo.UploadToQiniu(key, newFile, fileMeta.FileSize)
			if err != nil {

			}
			fileMeta.FileSha1 = ret.Hash
			fileMeta.Location = ret.Key
		} else {
			// 写入异步转移任务队列
